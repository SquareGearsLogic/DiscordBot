...
class DiscordEventBus():
  """TODO: Well, probably I'm reinventing a wheel, but I didn't get how to subscribe multiple listeners to a single discord.Client object\n
     ...
     Your plugin class must have "discordEventBusEvents" dictionary field that maps discord.Client event_names to your class methods"""
  ...
  def add(self, listener):
    """Add your plugin class object that contains "discordEventBusEvents" dictionary field before calling discord.Client::run()"""
    if self.isLocked or listener is None or hasattr(listener, 'discordEventBusEvents')==False or listener.discordEventBusEvents is None:
      return False
    for eventName in listener.discordEventBusEvents:
      self.listeners[eventName].append(listener)
    return True

  def remove(self, listener):
    """Remove your plugin class object that contains "discordEventBusEvents" dictionary field after your discord.Client is disconnected"""
  ...
  def _emit(self, event_name, args=None):
    ...
    for listener in self.listeners[event_name]:
      if not event_name in listener.discordEventBusEvents: continue
      listenerMethodName = listener.discordEventBusEvents[event_name]
      if args:
  ...
  def _bindEvents(self):
    ...
    self.listeners['on_raw_reaction'] = self.listeners['on_raw_reaction_add']=[]
    @self.client.event
